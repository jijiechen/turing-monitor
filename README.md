...
亮度被写成 0 了。`cmd 125`（保存设置）**未设置的字段会被写成 0**，而 `[8]` 是亮度；并且它
**重启后才生效**，所以当时看不出问题。恢复：

```sh
! /tmp/turzx/turzx brightness 100     # cmd 14，立即生效
# 若仍不正常，把设置整体写回合理值后重启设备
```

细节见 [docs/PROTOCOL.md](docs/PROTOCOL.md) 中的警告章节。
